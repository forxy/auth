...
abstract class PermissionDAOTest extends AbstractJUnit4SpringContextTests {
    ...
    void testGrantPermissionsToGroup() {
        permissionDAO.grantPermissionsToGroup(TEST_GROUP_CODE, TEST_CLIENT_ID, ['test1'] as Set)
        assert permissionDAO.getGroupPermissions(TEST_GROUP_CODE, TEST_CLIENT_ID) == ['test1'] as Set, 'Permissions not granted'
        permissionDAO.deleteClientPermissions(TEST_CLIENT_ID)
        assert !permissionDAO.getGroupPermissions(TEST_GROUP_CODE, TEST_CLIENT_ID), 'Permissions have not been removed'
    }

    @Test
    void testRevokeAccountPermissions() {
        permissionDAO.grantPermissionsToAccount(TEST_EMAIL, TEST_CLIENT_ID, ['test1', 'test2'] as Set)
        assert permissionDAO.getAccountPermissions(TEST_EMAIL, TEST_CLIENT_ID) == ['test1', 'test2'] as Set, 'Permissions not granted'
        permissionDAO.revokeAccountPermissions(TEST_EMAIL, TEST_CLIENT_ID, ['test2'] as Set)
        assert permissionDAO.getAccountPermissions(TEST_EMAIL, TEST_CLIENT_ID) == ['test1'] as Set, 'Permission "test2" should be revoked'
    ...
    void testRevokeGroupPermissions() {
        permissionDAO.grantPermissionsToGroup(TEST_GROUP_CODE, TEST_CLIENT_ID, ['test1', 'test2'] as Set)
        assert permissionDAO.getGroupPermissions(TEST_GROUP_CODE, TEST_CLIENT_ID) == ['test1', 'test2'] as Set, 'Permissions not granted'
        permissionDAO.revokeGroupPermissions(TEST_GROUP_CODE, TEST_CLIENT_ID, ['test2'] as Set)
        assert permissionDAO.getGroupPermissions(TEST_GROUP_CODE, TEST_CLIENT_ID) == ['test1'] as Set, 'Permission "test2" should be revoked'
        permissionDAO.deleteGroupPermissions(TEST_GROUP_CODE)
        assert !permissionDAO.getGroupPermissions(TEST_GROUP_CODE, TEST_CLIENT_ID), 'Permissions have not been removed'
    }

    @Test
    void testGetGroupsPermissionsUnion() {
        permissionDAO.grantPermissionsToGroup('testGroup1', TEST_CLIENT_ID, ['test1', 'test2'] as Set)
        permissionDAO.grantPermissionsToGroup('testGroup2', TEST_CLIENT_ID, ['test2', 'test3'] as Set)
        permissionDAO.grantPermissionsToGroup('testGroup3', TEST_CLIENT_ID, null)
        ...
        permissionDAO.deleteClientPermissions(TEST_CLIENT_ID)
        assert !permissionDAO.getGroupPermissions('testGroup1', TEST_CLIENT_ID), 'Permissions have not been removed'
        assert !permissionDAO.getGroupPermissions('testGroup2', TEST_CLIENT_ID), 'Permissions have not been removed'
        assert !permissionDAO.getGroupPermissions('testGroup3', TEST_CLIENT_ID), 'Permissions have not been removed'
    }
    ...
    void testRevokeAccountApprovals() {
        ...
        permissionDAO.deleteAccountApprovals(TEST_EMAIL)
        assert !permissionDAO.getAccountApprovals(TEST_EMAIL, TEST_CLIENT_ID), 'Approvals have not been removed'
    }
